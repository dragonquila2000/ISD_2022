					...
					?>
						<tr>
							...
							<td class="text-center">
                                <?php if($row['paid'] == 0): ?>
                                    <span class="badge badge-light">Chưa trả</span>
                                <?php else: ?>
							...
							<td class="text-center">
								...
								<?php elseif($row['status'] == 3): ?>
                                    <span class="badge badge-success">Đã đến nơi</span>
							...
							<td align="center">
								 <button type="button" class="btn btn-flat btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown">
				                  ...
				                  <div class="dropdown-menu" role="menu">
				                    <a class="dropdown-item" href="?page=orders/view_order&id=<?php echo $row['id'] ?>">Hóa đơn</a>
									<?php if($row['paid'] == 0 && $row['status'] != 4): ?>
				                    <a class="dropdown-item pay_order" href="javascript:void(0)"  data-id="<?php echo $row['id'] ?>">Đã trả tiền</a>
									<?php endif; ?>
...
<script>
	$(document).ready(function(){
		$('.delete_data').click(function(){
			_conf("Bạn có chắc chắn muốn xóa đơn hàng này không?","delete_order",[$(this).attr('data-id')])
		})
		$('.pay_order').click(function(){
			_conf("Bạn có chắc chắn về điều này? Nếu sai bạn sẽ phải bù tiền cho đơn hàng!! ?","pay_order",[$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})

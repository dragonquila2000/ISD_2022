			...
			success:function(resp){
				if(resp){
					...
					if(resp.status == 'success'){
						...
						var _frm = $('#login-frm')
						var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i>Sai tài khoản hoặc mật khẩu</div>"
						_frm.prepend(_msg)
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					$('#msg').html('<div class="alert alert-danger err_msg">Xảy ra lỗi!</div>')
					end_load()
				}
			}
		})
